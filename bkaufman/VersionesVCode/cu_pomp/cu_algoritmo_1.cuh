...
struct nuevos_params{
	...
	nuevos_params(T _mult){
		mult = _mult;
	}
	
	parametros<T> p_nuevos(parametros<T> promedios, parametros<T> viejos){
		parametros<T> result;

		for(int k=0; k<viejos.dim; k++){
			result.set_parametro(k, mult * (promedios.get_parametro(k) - viejos.get_parametro(k)) + viejos.get_parametro(k));
		}
...
class PropagoMC{//Clase para generar distintas perturbaciones a los parametros y simular los resultados hasta la proxima medicion.
	...
	void generoRandoms(){
		thrust::transform(thrust::device, c_state.begin(), c_state.end(), p.begin(), p.begin(), generador_parametros_aleatorios<T>());
	}
	...
	void calculoProbPuntos(){
		probabilidades_variables.resize(nPars);
		thrust::transform(thrust::device, v.begin(), v.end(), p.begin(), probabilidades_variables.begin(), datos.data()[dato_counter]);
	}
	...
	void reduzcoParametros(){
		...
		thrust::transform(thrust::device, count.begin(), count.end(), thrust::make_constant_iterator(dim), map.begin(), thrust::divides<T>());
		...
		nuevos_params<T> n_p(c_f.c_factor(dato_counter, pasada_counter));//Amplifico las perturbaciones por el cooling factor.
		parametros<T> nuevo_p = n_p.p_nuevos(p_promedio, p_viejo);//Coloco el nuevo centro de mis parametros.
	...
	void reduzcoVariables(){
		...
		//Reduzco con la probabilidad como peso.
		thrust::device_vector<float> probs_hechas_vector_padded(n * dim);
		thrust::gather(thrust::device, map.begin(), map.end(), probabilidades_variables.begin(), probs_hechas_vector_padded.begin());
		...
		//Tercera parte: transformo el vector media de parametros en un nuevo vector de parametros iniciales (todos iguales).
		v_viejo = calc_est.get_media();//Creo e inicializo el estado promedio del sistema con el vector de la media.
		
		thrust::fill(thrust::device, v.begin(), v.end(), v_viejo);
	}

	void reduzcoVariablesSinPeso(){
		int n = nVars;
		int dim = dimVars;
